...
class LPR:
    ...
    def process(self,input):
        ...
        for i,box in enumerate(boxes):
            ...
            str_n = len(string_LP)
            if (str_n > 5):
                temp = '{' + string_LP[str_n-4] + string_LP[str_n-3] + string_LP[str_n-2] + string_LP[str_n-1] + '}'
                self.draw_string(input , box[0], box[1] ,temp,color=(225,0,0))
                yep_num = 0
                ...
                #         if temp in i :
                #             yep_num += 1
                #     if yep_num == 3 and old_car not in new_car:
                #         old_car = new_car
                #         ser.write(temp.encode("utf-8"))
